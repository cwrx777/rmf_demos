...
class TaskRequester(Node):
    ...
    def __init__(self, argv=sys.argv):
        ...
        parser.add_argument('-t', '--task-json', type=str,
                            help='File containing the task definition',
                            required=True)
        parser.add_argument('-id', '--task_id',
                            help="Task ID")
        ...
        with open(self.args.task_json, "r") as f:
            rmf_task_request = json.load(f)

        self.get_logger().info(f"Task request: {rmf_task_request}")
        ...
        if self.args.task_id is not None:
            msg.request_id = self.args.task_id
        else:
